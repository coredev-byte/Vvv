  ...
  if (matchDataStatus === "riot_production_required" && HENRIKDEV_ENABLED && HENRIKDEV_API_KEY) {
    try {
      ...
      if (hRes.ok) {
        ...
        const rawList = hData.data || [];
        matches = rawList.map((m) => normalizeHenrikMatch(m, identity.puuid)).filter(Boolean);
        matchDataStatus = matches.length > 0 ? "ok_henrikdev" : "henrikdev_empty";
        henrikDebug = { httpStatus: hRes.status, rawCount: rawList.length, keptCount: matches.length };
      } else {
        const errText = await hRes.text().catch(() => "");
        henrikDebug = { httpStatus: hRes.status, errorBody: errText.slice(0, 300) };
      }
    } catch (err) {
      henrikDebug = { exception: err.message };
